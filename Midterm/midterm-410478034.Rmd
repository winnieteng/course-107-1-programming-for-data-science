...
```{r ans18}
maleGrade <- which(hwGradeSubsample$女性 == FALSE)
colMeans(hwGradeSubsample[maleGrade,6])
```
...
```{r ans22}
hwSubmit3 <- hwSubmit[which(hwSubmit$作業代碼 == 'Exercise3'),]
deadline <- ymd_hms('2018-11-06 00:00:00',tz="Asia/Taipei")
hwSubmit3$截止時差 <- round(difftime(deadline, hwSubmit3$Timestamp, units="hours"))
hwSubmit3
```
...
```{r ans23}
female_time <- mean(hwSubmit3$截止時差[which(hwSubmit3$女性 == TRUE)]) 
male_time <- mean(hwSubmit3$截止時差[which(hwSubmit3$女性 == FALSE)])
female_time - male_time
```
