...
params:
  studentID: "410478034"
  studentName: "鄧維妮"
  repoURL: "https://github.com/winnieteng/inclassPractice"
---
...
```{r ans13}
ans <- str_which(colnames(gsHW2ws),"ans")
colMeans(gsHW2ws[ans] ,na.rm = TRUE)
```

## 1.4
透過`gsHW2ws$答對率 <- ...` 在gsHW2ws新增一個欄位變數叫「答對率」, 其值為每個人各自在5題的正確比率。（hint: `rowSums(x, na.rm=TRUE)`) 
```{r ans14}
gsHW2ws$答對率 <- rowMeans(gsHW2ws[ans], na.rm=TRUE)
```

## 1.5
作業成績為5分加上「答對率」* 5。請在gsHW2ws新增「成績」欄位，並填入每個人的成績值。  
```{r ans15}
ans_rate <- gsHW2ws$答對率 <- rowMeans(gsHW2ws[ans], na.rm=TRUE)
gsHW2ws$成績 <- 5+ans_rate*5
```

## 1.6
答對4題的人，最常錯的問題是那兩題？請找出其題目代碼。(`min()`與`max()`可找出極小與極大值，而`which()`可找出符合設定條件的元素位置)
```{r ans16}
q4 <- gsHW2ws[which(gsHW2ws$成績==9),]
which(colSums(q4[ans]) != max(colSums(q4[ans])))
```

## 1.7
答對3題的人，有那幾題正確率低於5成？請找出其題目代碼。
```{r ans17}
q3 <- gsHW2ws[which(gsHW2ws$成績==8), ]
which(colMeans(q3[ans], na.rm=TRUE) < 0.5)
```
...
allBankData$年月 %>% 
  str_c("1", . ,"/01") %>%
  lubridate:: ymd() %>%
  { . + lubridate:: years(911) } -> allBankData$年月
```
...
exData$期間 %>% 
  str_replace( . ,"M", "") %>%
  str_c( . ,"01") %>%
  lubridate:: ymd() -> exData$期間
exData %>% 
  mutate(
    `印尼盾IDR/USD`=as.numeric(`印尼盾IDR/USD`),
...
```{r ans33}
str_locate(colnames(exData), "USD/")

colnames(exData)[c(4,10,15)] <- str_sub(colnames(exData[c(4,10,15)]),1,2)
exData
```

```{r}
#將全部匯率都換的方式
colnames(exData) -> cNames 
currencyName <- cNames[-1]
currencyName %>% str_locate("/") %>%
  {str_sub(currencyName,1,.[,1]-4)} -> 
  currencyName
colnames(exData)[-1]<-currencyName
exData
```
