...
```{r ans12}
mutate(subsample, 及格=學期成績 >= 60) -> subsample
summarise(
  subsample,
  總修習學分數=sum(學分數),
  總實得學分數=sum(學分數[及格])
...
totalCredits <- function(x){
  id_data <- transcriptData[which(transcriptData$學號==x), ]
  id_pass <- id_data$學期成績 >= 60
  summarise(
  id_data,
  學號=x,
  總修習學分數=sum(學分數),
  總實得學分數=sum(學分數[id_pass])
  )
}
...
```{r ans15}
mutate(transcriptData, 系別=str_sub(學號,5,6) -> subsample) -> transcriptData
```
...
```{r ans16}
transcriptData$科目名稱[which(transcriptData$學年==100 & transcriptData$學期==1 & transcriptData$`必選修類別（必∕選∕通）`=='通')] -> classtype
通識科目總表 <- unique(classtype)
```
...
```{r ans18}
transcriptData$系別[which(transcriptData$科目名稱 == 'ZT13' & transcriptData$學年 == 104 & transcriptData$學期==1)] -> department
shannon(department)
```
