...
```{r ans11}
library(stringr)
classData$`公-10-06` <- as.factor(str_replace_all(classData$`公-10-06`,c("1"="有看公告","0"="沒看公告")))
```
...
```{r ans13}
length(which(classData$`公-10-06` == "有看公告" & classData$`練2018-10-17-1` ==1)) / length(which(classData$`公-10-06` == "有看公告"))
```
...
```{r ans14}
length(which(classData$`公-10-06` == "沒看公告" & classData$`練2018-10-17-1` ==1)) / length(which(classData$`公-10-06` == "沒看公告"))
```
...
```{r ans25}
table(hwData[which(hwData$作業代碼 == "hw1" & difftime(deadline, hwData$Timestamp, units = "days")>0),3])
```
